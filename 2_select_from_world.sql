...
SELECT
  name,
  (population/1000000) AS population_milli
FROM world
...
/* 9. For South America show population in millions and GDP in billions both to
      2 decimal places. */
SELECT
  name,
  ROUND(population/1000000, 2) AS population_milli,
  ROUND(gdp/1000000000, 2) AS gdp_billi
FROM world
WHERE continent LIKE 'south america';

/* 10. Show the name and per-capita GDP for those countries with a GDP of at
       least one trillion (1000000000000; that is 12 zeros). Round this value to the
       nearest 1000. */
SELECT
  name,
...
FROM world
WHERE
  LEFT(name, 1) LIKE LEFT(capital, 1)
  AND name != capital;
